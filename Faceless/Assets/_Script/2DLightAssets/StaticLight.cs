	...
	void setLight () {
		...
		//for (int m = 0; m < 1; m++) {
			...
			for (int i = 0; i < mf.GetTotalPointCount(); i++) {								   // ...and for ever vertex we have of each mesh filter...
				...
				RaycastHit2D ray = Physics2D.Raycast(transform.position, worldPoint - transform.position, (worldPoint - transform.position).magnitude,staticLayer);


				if(ray){
					v.pos = ray.point;
					//v.pos = ray.point + ((ray.point - new Vector2(transform.position.x,transform.position.y)).normalized*(Vector2.Distance(ray.point, new Vector2(transform.position.x,transform.position.y))+1f));
					//v.pos = ray.point + ((ray.point - new Vector2(transform.position.x,transform.position.y))*0.1f);
			...
			if(tempVerts.Count > 0){
				...
				for(int r = 0; r<2; r++){
					...
					if(isEndpoint == true){
						...
						RaycastHit2D rayCont = Physics2D.Raycast(fromCast, dir, mag,staticLayer);
						...
						if(rayCont){
							hitp = rayCont.point;
							//hitp = rayCont.point + ((rayCont.point - new Vector2(fromCast.x,fromCast.y))*0.1f);
							//hitp = rayCont.point + ((rayCont.point - new Vector2(fromCast.x,fromCast.y)).normalized*(Vector2.Distance(rayCont.point, new Vector2(fromCast.x,fromCast.y))+1f));
						}else{
		...
		for (int i = 0; i < lightSegments; i++)  {
			...
			RaycastHit2D ray = Physics2D.Raycast(transform.position,v.pos - transform.position,lightRadius,staticLayer);
